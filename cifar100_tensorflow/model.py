...
class InceptionResnet:
    def __init__(self, hparam: HParam, training):
        ...
        self.encode = output
        self.logits = logits
        self.losses = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(y, logits))
        self.predict = tf.argmax(tf.nn.softmax(self.logits), axis=1, output_type=tf.int32)
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(y, tf.reshape(self.predict,shape=(-1,1))),dtype=tf.float32))
        if self.training:
            ...
            with tf.name_scope("train"):
                tf.summary.scalar("acc", self.accuracy,collections=["train_summary"])
                tf.summary.scalar("loss", self.losses,collections=["train_summary"])
            self.summary_op = tf.summary.merge_all(key="train_summary")
            with tf.name_scope("test"):
                tf.summary.scalar("acc", self.accuracy,collections=["test_summary"])
                tf.summary.scalar("loss", self.losses,collections=["test_summary"])
            self.test_summary_op = tf.summary.merge_all(key="test_summary")
